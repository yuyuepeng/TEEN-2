...
@property(nonatomic,strong)FHeadModel *model;

@end

@implementation FindViewController1
...
-(void)createHeadWithModel:(FHeadModel *)model{
    ...
    UITableView *tv = (id)[self.view viewWithTag:101];
...
-(void)loadData{
    ...
    _page = 1;
        [self.manager GET:DeSection parameters:@{@"id":@(self.model1.Kid),@"page":@(_page)} success:^(AFHTTPRequestOperation *operation, id responseObject) {
            ...
            self.model = [[FHeadModel alloc]initWithDictionary:responseObject[@"data"][@"head"] error:nil];
            [self createHeadWithModel:self.model];
            UITableView *tv = (id)[self.view viewWithTag:101];
            [tv reloadData];
